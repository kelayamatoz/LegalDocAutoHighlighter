...
def findSuitableNumTopics(bowCorpus, trainDict):
    ...
    trainInd = sorted(random.sample(xrange(len(bowCorpus)), trainSize))
    testInd = sorted(set(xrange(len(bowCorpus))) - set(trainInd))
...
def getTopicsWithNormalizedWeight(model):
    ...
    norm = 0
    for topic, val in topicCoherencePair.iteritems():
        norm += val
